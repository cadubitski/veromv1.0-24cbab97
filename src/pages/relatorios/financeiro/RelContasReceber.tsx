...
export default function RelContasReceber() {
  ...
  const { data: rawData = [], isFetching } = useQuery({
    queryKey: ["rel_contas_receber", companyId, dateFromIssue, dateToIssue, dateFromDue, dateToDue],
    enabled: !!companyId,
    queryFn: async () => {
  ...
  }, [rawData, filterStatus, filterSource, filterClient, filterDoc, filterDesc]);
  ...
  const clearFilters = () => {
    ...
    setFilterStatus("all"); setFilterSource("all"); setFilterClient(""); setFilterDoc(""); setFilterDesc("");
  };

  const hasFilters = dateFromIssue || dateToIssue || dateFromDue || dateToDue || filterStatus !== "all" || filterSource !== "all" || filterClient || filterDoc || filterDesc;
    ...
    <DashboardLayout>
      <div className="space-y-6 print:space-y-4">
        <div className="flex items-start justify-between gap-4 print:hidden">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Relatório de Contas a Receber</h1>
            <p className="text-sm text-muted-foreground mt-1">Visão completa dos títulos a receber com auditoria detalhada</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="border-l-4 border-l-amber-500">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center justify-between">
                <div><p className="text-xs text-muted-foreground uppercase tracking-wide">A Receber</p><p className="text-xl font-bold text-amber-600 dark:text-amber-400">{fmtMoney(totalPending)}</p></div>
          ...
          <Card className="border-l-4 border-l-emerald-500">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center justify-between">
                <div><p className="text-xs text-muted-foreground uppercase tracking-wide">Recebido</p><p className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{fmtMoney(totalPaid)}</p></div>
