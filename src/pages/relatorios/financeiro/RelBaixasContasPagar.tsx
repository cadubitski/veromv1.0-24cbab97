...
export default function RelBaixasContasPagar() {
  ...
  const { data: rawData = [], isFetching } = useQuery({
    queryKey: ["rel_baixas_pagar", companyId, dateFromPaid, dateToPaid, dateFromDue, dateToDue],
    enabled: !!companyId,
    queryFn: async () => {
  ...
  }, [rawData, filterSource, filterVendor, filterDoc, filterAccount]);
  ...
  const handleExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Baixas CP");
    XLSX.writeFile(wb, `baixas-contas-pagar-${format(new Date(), "yyyy-MM-dd")}.xlsx`);
  };

  const clearFilters = () => {
    setDateFromPaid(""); setDateToPaid(""); setDateFromDue(""); setDateToDue("");
    setFilterSource("all"); setFilterVendor(""); setFilterDoc(""); setFilterAccount("all");
  };

  const hasFilters = dateFromPaid || dateToPaid || dateFromDue || dateToDue || filterSource !== "all" || filterVendor || filterDoc || filterAccount !== "all";
    ...
    <DashboardLayout>
      <div className="space-y-6 print:space-y-4">
        <div className="flex items-start justify-between gap-4 print:hidden">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Baixas de Contas a Pagar</h1>
            <p className="text-sm text-muted-foreground mt-1">Relatório auditável de todos os pagamentos liquidados</p>
          </div>
        ...
        <div className="hidden print:block text-center border-b pb-4 mb-4">
          <h1 className="text-xl font-bold">Baixas de Contas a Pagar</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="border-l-4 border-l-rose-500">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center justify-between">
                <div><p className="text-xs text-muted-foreground uppercase tracking-wide">Total Pago</p><p className="text-xl font-bold text-rose-600 dark:text-rose-400">{fmtMoney(totalBaixas)}</p></div>
                <CheckCircle2 className="h-8 w-8 text-rose-500 opacity-60" />
              </div>
            </CardContent>
          </Card>
                      ...
                      <tr key={t.id} className={`border-b transition-colors hover:bg-muted/30 ${idx % 2 === 0 ? "" : "bg-muted/10"}`}>
                        {ALL_COLUMNS.filter((c) => visibleCols.has(c.key)).map((c) => (
                          <td key={c.key} className="px-4 py-2.5 align-middle whitespace-nowrap">
                            {c.key === "amount" ? (
                              <span className="font-semibold text-rose-600 dark:text-rose-400">{fmtMoney(Number(t.amount))}</span>
                            ) : (
                    ...
                    <tr className="border-t-2 bg-muted/30 font-semibold">
                      {ALL_COLUMNS.filter((c) => visibleCols.has(c.key)).map((c, i) => (
                        <td key={c.key} className="px-4 py-3 text-sm">
                          {i === 0 ? <span className="text-muted-foreground">TOTAL</span> : c.key === "amount" ? <span className="text-rose-600 dark:text-rose-400">{fmtMoney(totalBaixas)}</span> : ""}
                        </td>
                      ))}
                    </tr>
                  </tfoot>
